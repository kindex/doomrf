...
const
  origlev:tcapt='FL03'; lever00:tcapt='FLEV';
  maxallpl:integer=4;
...
{   tnpat=0..maxpat;}
  ...
  tmon=object(tobj)
     life,ai,know,see,barrel,aqua: boolean;
     target: record x,y,mon:integer end;
     who,lastwho: integer{0..maxmon};
     dest: tdest;
     tip: tmaxmontip;
     delay,freez,fired,deldam,statedel,vis,savedel,hero,god,longjumpn,jumpfr,
        armorhit,armorbomb,armoroxy,armorfreez,qdamage:longint;
...
begin
  onhead:=(abs(ax-getcx)<getsx*8)and
   ( (abs((getcy-getsy*4)-ay)<8)or
    (abs((getcy+getsy*4)-ay)<8));
end;
...
begin
  ...
  for i:=1 to ml do begin
    if l[i].time>(mfps/speed) then begin
      print(ax,ay+y, l[i].c, l[i].s);
      inc(y,h);
    end
    else begin
      c:=getcolor(
      round(pal[l[i].c*4+2]*l[i].time/mfps*speed),
      round(pal[l[i].c*4+1]*l[i].time/mfps*speed),
      round(pal[l[i].c*4+0]*l[i].time/mfps*speed));

      print(ax,ay+y,c, l[i].s);
      inc(y,round(h*l[i].time/mfps*speed));
    end
  end;
end;
...
begin
  ...
  if it[nn].armorhit<>0 then begin
    inc(armorhit,round(it[nn].armorhit*mfps/speed));
    ok:=true;
  end;
  if it[nn].armorbomb<>0 then begin
    inc(armorbomb,round(it[nn].armorbomb*mfps/speed));
    ok:=true;
  end;
  if it[nn].armoroxy<>0 then begin
    inc(armoroxy,round(it[nn].armoroxy*mfps/speed));
    ok:=true;
  end;
  if it[nn].armorfreez<>0 then begin
    inc(armorfreez,round(it[nn].armorfreez*mfps/speed));
    ok:=true;
  end;
  if it[nn].qdamage<>0 then begin
    inc(qdamage,round(it[nn].qdamage*mfps/speed));
    ok:=true;
  end;
...
begin
  ...
  setdelay(round(mfps/speed*0.25));
end;
...
{   if ((rtimer.hod mod 2)=(hero mod 2))then }see:=seeany;
   if (((rtimer.hod+2) mod round(20/speed))=(hero mod round(20/speed)))then  goal:=getgoal;
...
var
  l:shortint;
  sp,per,ry,d,ty,s , rx, sx,sy, px,py, tp:real;
  i,j,k:longint;
begin
  ...
  setdelay(round(weapon[nweap].shot*mfps/speed));
...
{    if ty>abs(target.x-x)/d*}
  ...
  if ai and onhead(target.x, target.y) and level.sniper{and (abs(getcx-target.x)<getsx*8)} then
  begin
    rx:=0;
  ...
  begin
    ...
    for i:=1 to rfunit.bul[weapon[nweap].pul].shot do begin
      px:=sp*(1+random*per-per/2)*rx+dx;
      py:=sp*(random*per-per/2)+sp*ry-weapon[nweap].speedy;
{      if ai then begin
         tp:=px; px:=py; py:=tp;
      end;}
      map.initbul(x+l*getsx*4,
      y-monster[tip].h,
      px,py,weapon[nweap].pul,who, boolean(qdamage<>0));
    end;
    setstate(fire,0.1);
  end;
  end;
  ...
  if upr then
    if (bul[tip].time=0)or bul[tip].walldetonate then begin
      for i:=0 to random(5)+5 do
        map.randompix(x-8*l{-sdx},y+dy,0,0,5,5,blow);
  ...
  for i:=0 to maxmon do
   ...
   begin
    ...
    begin
      ...
      bul[tip].freez/speed*mfps*qdamage,
...
begin
  ...
  if (level.multi)and(not level.death)and(hero>0)and(player[hero].bot>0)and (map.m^[dwho].hero>0) then begin
...
begin
  ...
  if armoroxy<>0 then coxy:=0;
  if armorfreez<>0 then afreez:=0;
...
{  if fired=0 then }
  ...
  if ai and (dwho>=0)and ((target.mon=0)or not see)then begin
    target.mon:=dwho;
  end;
...
{  if life then }clearwall(cwall);
  ...
  begin
    inc(delay,2);
    if delay>truptime*mfps/speed then
    begin
      done;
      exit;
    end;
...
{    ty:=round(ry*8);}
    ...
    case level.sniper of
     false: if (abs(target.x-tx*8)<16)and(abs(target.y-ty*8)<32{+abs(target.x-tx*8)div 8})then
     begin
       see:=true;
       break;
     end;
...
begin
  ...
  with map do
  if rtimer.hod mod settings.botframe=(who mod settings.botframe) then begin
    for i:=1 to level.maxpl do begin
      ...
      if (d<min)and see then begin min:=d; target.mon:=j; end;
    end;

    if  target.mon>0 then begin
...
//  if (not see)and not level.look and (random(300)=0)and not inwater then know:=false;
  ...
  begin
    ...
    if not see then
     if (abs(target.x-self.getcx)>10) then
     begin
      if target.x<self.getcx then include(key,kleft) else include(key,kright);
     end
     else
...
//  if standing then k:=1 else k:=0.7;
  ...
  if dx<0 then dx:=dx+monster[tip].brakes*ms{2}*sqrt(usk)*speed/mfps
  else
   dx:=dx+monster[tip].acsel*ms{2}*sqrt(usk)*speed/mfps;
...
{  log('tip',tip);
  ...
  c:=0;
  if (armorhit<>0)then c:=blue;
  if (armorbomb<>0)then c:=red;
  if (armoroxy<>0)then c:=green;
  if (armorfreez<>0)then c:=green;
  ...
  case state of
   ...
   run:  p[monster[tip].run[vis,dest]].spriteo(mx-ax,my-ay,c);
   fire: p[monster[tip].fire[dest]].spriteo(mx-ax,my-ay,c);
   hack: p[monster[tip].damage[dest]].spriteo(mx-ax,my-ay,c);
   die:  p[monster[tip].die[vis,dest]].spritesp(mx-ax,my-ay);
   crash:p[monster[tip].bomb[vis,dest]].spritesp(mx-ax,my-ay);
   hai:  p[monster[tip].hai[dest]].spriteo(mx-ax,my-ay,c);
   duck:  p[monster[tip].duck[dest]].spriteo(mx-ax,my-ay,c);
  end;
//  dec(my);
...
//    bot[i]:=tip[i];
  ...
  if level.maxpl>0 then men[1]:='Без ботов'
  else men[1]:='отмена';
  men[2]:='1 бот';
  men[3]:='2 бота';
  men[4]:='3 бота';
  ...
  men[8]:='7 ботов';
  men[9]:='8 ботов';
  level.maxpl:=menu(bots+1,bots+1,'Боты')-1;
...
begin
  ...
  case s of
   ...
   3: begin reswap:=false;reswaptime:=90; monswaptime:=120; rail:=false; look:=true; sniper:=false; end;
   4: begin reswap:=false;reswaptime:=120;monswaptime:=60;  rail:=false; look:=true; sniper:=true; end;
...
procedure movemouse;
begin
    mx:=mouse.x;  my:=mouse.y; push:=mouse.push; push2:=mouse.push2;push3:=mouse.push3 or pkey[59];
//    add:=7;
    {    case res of
     0,1: add:=1;
     2,3: add:=7;
     ...
     if not level.death then
      begin
       if pos('god',keybuf)>0 then
         begin
           for i:=1 to level.maxpl do
            if map.m^[player[i].hero].god=0 then begin
              map.m^[player[i].hero].god:=-1;
...
if ioresult<>0 then begin writeln('WARNING: doom.ini not fund; default mod = RF'); exit; end;
  ...
  begin
    ...
    if i>0 then begin
      ...
      if s1='botdelay' then runmod:=s2;
      if s1='mondelaytitle' then title:=s2;
    end;
  end;
  close(f);
...
//  initgraph(res);
 ...
 repeat
  ...
  unpush:=false; scrmode:=normal; hod:=0;  speed:=1;
  repeat
...
//    rb.print(getmaxx-24,getmaxy-16,st0(round(speed*10),3));
