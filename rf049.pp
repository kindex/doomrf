...
const {(C) DiVision: kIndeX , Zonik , Dark Sirius }
  game='Richtiger Faschist (Fuhrer Ruckkehr)';
  version='1.49';
  data='12.1.2002';
  title:string='DOOM 513' {+version+' ['+data+']'};
  comment='Beta version *** Freeware';
...
begin
  ...
  a:=winbmp;
  b:='Теперь можно отпразновать победу';
  del:=5;
  clear;
  box(0,0,getmaxx,getmaxy);
  c:=1;
...
begin
  ...
  if (tip=13)or(tip=12)or(tip=25) then
...
begin
  ...
  if bot=0 then begin // Only human
    ...
    if drugs.en then begin
      begin
        k:=random(100);
//        k:=round(drugs.getest/5);
        port[$3c8]:=0;
        for i:=0 to 255 do
        begin
...
begin
  ...
  begin
    if inwater or ((g=0)and not standing)then dy:={dy+}monster[tip].jumpy*0.5*usk
    else
...
{  if standing and not (kleft in key) and not (kleft in key)then begin
  ...
  if jumpfr=0 then
    if kjump in key then begin
     if (inwater) or ((g=0)and not standing) then
...
begin
  ...
  if (abs(sy-ey)>4)and not clever then see:=false;
...
//  if (not see)and not level.look and (random(300)=0)and not inwater then know:=false;
  ...
  begin
    ...
    if (weap=1)or((fired.en)and(clever)) then if target.x<self.getcx then include(key,kleft) else include(key,kright);
  end;
...
begin
  ...
  getfattr(ff,attr);
  if attr and readonly>0 then begin error('Нельзя изменять стандартные уровни!'); exit; end;
{$i-}  rewrite(ff,1); {$i+}
...
begin
  ...
  s:=level.name[level.cur]+' - '+strtime(round(time.tik/tpr))+' - Прошло - '+strtime(round((level.curtime-level.start)/tpr));
...
//  initgraph(res);
 ...
 repeat
  ...
  unpush:=false; scrmode:=normal; hod:=0;  level.start:=time.tik; speed:=1; si:=false;
  level.alltime:=0;
